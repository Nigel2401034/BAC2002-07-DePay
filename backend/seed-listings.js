...
const DEFAULT_SELLER_WALLET = '0x0000000000000000000000000000000000000000';
...
async function seedListingsOnStartup() {
  ...
  try {
    ...
    throw new Error(`Invalid JSON in baselistings/listings.json: ${error.message}`);
  }
  ...
  const seededBy = (process.env.SEED_SELLER_WALLET || DEFAULT_SELLER_WALLET).toLowerCase();
  ...
  for (let i = 0; i < seedListings.length; i += 1) {
    ...
    const listingData = {
      ...
      imageCid: item.imageCid ? String(item.imageCid).trim() : (existingDoc && existingDoc.imageCid ? existingDoc.imageCid : null),
      ipfsCid: item.ipfsCid ? String(item.ipfsCid).trim() : (existingDoc && existingDoc.ipfsCid ? existingDoc.ipfsCid : null),
      sellerWallet: seededBy,
      source: 'seed'
    };

    if (!listingData.imageCid && allowIpfsUpload && listingData.imageFile) {
      const imagePath = path.join(__dirname, '../baselistings', listingData.imageFile);
      if (fs.existsSync(imagePath)) {
        try {
          listingData.imageCid = await uploadFilePathToIPFS(imagePath, listingData.imageFile);
        } catch (error) {
          console.log(`⚠️  Image upload failed for ${seedKey}. Continuing without image CID.`);
        }
      }
    }
    ...
    if (!listingData.ipfsCid && allowIpfsUpload) {
      try {
        ...
        console.log(`⚠️  IPFS upload failed for ${seedKey}. Continuing without CID.`);
      }
    } else if (listingData.ipfsCid) {
      console.log(`ℹ️  Using existing CID for ${seedKey}: ${listingData.ipfsCid}`);
    }
  ...
  console.log(`🌱 Seed completed: inserted=${insertedCount}, updated=${updatedCount}, total=${seedListings.length}`);
}
